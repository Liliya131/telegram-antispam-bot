...
class AbstractStorage(ABC):
    ...
    @abstractmethod
    def get_user_confirm_code(self, group_id: int, user_id: int) -> Optional[str]:
        pass
...
class FileSystem(AbstractStorage):
    ...
    def __init__(self, path: str, groups_list: set[int] = None):
        self.storage_dir = convert_path(path)
        self.groups_list = set([] if groups_list is None else [int(x) for x in groups_list])
        self._load_groups_list()
        self._create_storage_dir()
        self._create_groups_dir()

    def _load_groups_list(self) -> None:
        file_path = to_path(self.storage_dir, self.groups_list_file)
        if os.path.exists(file_path):
            with codecs.open(file_path, "r", encoding="utf-8") as file:
                self.groups_list = {int(x.strip()) for x in file.readlines() if x.strip()}
    ...
    def is_user_confirmed(self, group_id: int, user_id: int) -> bool:
        return os.path.exists(
            to_path(self.storage_dir, str(group_id), self.confirmed_dir, str(user_id))
        )

    def set_user_confirmed(self, group_id: int, user_id: int) -> bool:
        confirmed_file = to_path(self.storage_dir, str(group_id), self.confirmed_dir, str(user_id))
        confirm_code_file = to_path(self.storage_dir, str(group_id), self.confirm_codes_dir, str(user_id))
        with codecs.open(confirmed_file, "w", encoding="utf-8"):
            pass
        try:
    ...
    def set_user_confirm_code(self, group_id: int, user_id: int, confirm_code: str) -> None:
        file_name = to_path(self.storage_dir, str(group_id), self.confirm_codes_dir, str(user_id))
    ...
    def on_added_to_group(self, group_id: int) -> None:
        ...
        self.groups_list.add(int(group_id))
        self._save_groups_list()

    def _save_groups_list(self) -> None:
        with codecs.open(to_path(self.storage_dir, self.groups_list_file), "w", encoding="utf-8") as handle:
    ...
    def _create_group_dir(self, group_id: int) -> None:
        for dir_path in self.required_dirs:
            create_storage_dir(to_path(self.storage_dir, str(group_id), dir_path))
